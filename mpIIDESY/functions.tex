...
PEDE Inputs for Circle Fit and Misalignment ($M$) as a rotation ($\theta$) along the detector centre,
with $x=x(\theta)=??$ and $z=z(\theta)=??$, so that $dR/d\theta$ can be calculated substituting for $x=x(\theta)=$ and $z=z(\theta)=$ into: 
\begin{equation}	
\mathrm{Residual}= R =\mathrm{Point \ To \ Line \ DCA} - \mathrm{Drift \ Circle \ Radius} = DCA(z(\theta),x(\theta),m,c) - r = \frac{ |c+mz-x| }  { \sqrt{m^2+1} } -r
\end{equation}
