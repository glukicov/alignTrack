...
\begin{center}
	\vspace{1cm}
	{\huge Tracker Alignment for g-2 experiment: \\ Summary of 2D MC and PEDE results}\\
	\vspace{1.5cm}
...
\clearpage
\section{Tracker Alignment}\label{sec:align}
...
\begin{enumerate}
	...
	\item Define the misaligned geometry (i.e. actual straw coordinates due to misalignment, which is set in the MC).
...
\end{figure}
\vspace{-0.35cm} 
\subsection{Current Alignment Work}
\vspace{-0.2cm}
At the moment, I am implementing a \textit{circle fit} on the simulated hit data. This is done to match the reconstruction of the real data, where only the hit times are known, from which, given a know drift velocity (see Fig.~\ref{fig:drift}), a distance (i.e. the radius of the drift circle) can be reconstructed. The ambiguity in the trajectory is resolved by combining two drift circles to form a doublet, and the 2D hit position is resolved by combining two doublets. Multiples 2D position from many modules give a 3D track.
After this, the following features will be implemented in the MC code:
\vspace{-0.2cm}
\begin{itemize}
\setlength\itemsep{-0.5em}
\item Tracker Alignment in 1D (full tracker station in 3D with 1D misalignment, \textit{art} geometry, no magnetic field (MF), no multiple scattering (MS), straight tracks, realistic hit rejection).
\item Tracker Alignment in 2D (misalignment in 2D, MS, efficiency effects, no MF).
\item Tracker Alignment 3D with MF (misalignment in 3D, MF with curved tracks, using General Broken Line package~\cite{GBL}).
\end{itemize}
\clearpage
\begin{figure}[!ht]
	\centering
...
\subfloat[]{\includegraphics[scale = 0.40]{fig/circles.png}}
    \vspace{-0.1cm}
    \caption{a) Time versus distance relation in a single straw predicted by \texttt{GARFIELD}~\cite{GARFIELD} simulation for 50:50 Ar:Ethane at a gain of $10^6$ for a 1.5 T magnetic field~\cite{g-2_TDR}. b) Truth hit drift circles in a single module.}
\label{fig:drift} 
\end{figure}
\vspace{-0.4cm}
...
\clearpage
\subsection{2D Misalignment in x and z via generic rotations.}
...
Collating the results from the previous 2 sections we can easily write down the required inputs to PEDE: 
...
\chapter{Integration with \textit{art}}
