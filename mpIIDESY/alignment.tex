...
m_i = \frac{p_i}{|\bf{p}|}, 	
\end{equation}
where $p_i$ is the momentum in $i$ direction, and $\bf{p}$ is the momentum vector. 

A line in 3D can be parametrised with a variable parameter $t$ as follows
\begin{equation}
\bf{r}(t) = \begin{pmatrix}x\\y\\z\\\end{pmatrix} = \begin{pmatrix}x_L + m_x  t\\y_L + m_y t\\z_L + m_z t\\\end{pmatrix},
\end{equation}
where ($x_L, y_L, z_L$)  is some point on the line. However, we only need 4 parameters, out of 6 defined above, to specify the line. A 3D line is defined by two points, each with 3 DoF, giving 6 DoF for the line. The line, however, is invariant under the translation of either of the points, giving 4 DoF. Assuming the track is not parallel to the X-Y plane, as will be the case for all tracks of interest here, we can remove $z_L$, and $m_z$ dependence, by setting $z_0=0$, and dividing by $m_z$. 
\begin{equation}
\bf{r}(t) = \begin{pmatrix}x\\y\\z\\\end{pmatrix} = \begin{pmatrix}x_L + \frac{p_x}{p_z}  t\\y_L + \frac{p_y}{p_z}  t\\  t\\\end{pmatrix} = \begin{pmatrix}x_L + p_x^{'} t\\y_L + p_y^{'}  t\\  t\\\end{pmatrix},
\end{equation}
where $x_L$ and $y_L$ are point where track intersected the Z=0 plane, in the future, we drop the $^{'}$ notation on momentum components.
...
\begin{equation}
DCA^2 = (x_L - x_P + p_xt_{min})^2 + (y_L - y_P + p_yt_{min})^2  + (-z_P + t_{min})^2.
\label{eq:t}
\end{equation}
The distance is minimised, by taking the residual w.r.t to $t_{min}$:
\begin{equation}
\frac{\partial d^2}{\partial t_{min}} = 0, 
\end{equation}
and rearranging, to find
\begin{equation}
 t_{min} = \frac{p_x x_p + p_yy_p +z_p-p_xx_l-p_yy_l}{p^2_x+p^2_y+1}.
\end{equation}
The above expression for $t_{min}$ can be substituted into Eq.~\ref{eq:t}
\begin{equation}
\begin{split}
...
\end{split}
\end{equation}
This allows us to write the derivatives of the residual w.r.t to local (track) parameters: $ p_x, p_y, x_L, x_L$, and global (detector position) parameters: $x_P, y_P, z_P$.

First, let's consider the derivatives of $t_{min}$ w.r.t to the above parameters:

\begin{equation}
\frac{\partial t_{min}}{\partial x_L} = \frac{-p_x}{p_x^2+p_y^2+1}
\end{equation}

\begin{equation}
\frac{\partial t_{min}}{\partial y_L} = \frac{-p_y}{p_x^2+p_y^2+1}
\end{equation}

\begin{equation}
\begin{split}
\frac{\partial t_{min}}{\partial p_x} = \frac{x_p-x_l}{p_x^2+p_y^2+1}-\frac{2 p_x \left(-p_xx_l-y_l p_y+p_x x_p+p_y y_p+z_p\right)}{\left(p_x^2+p_y^2+1\right){}^2} = \\ 
=\frac{x_p-x_l}{p_x^2+p_y^2+1}-\frac{2p_xt_{min}}{p_x^2+p_y^2+1}
\end{split}
\end{equation}

\begin{equation}
\begin{split}
\frac{\partial t_{min}}{\partial p_y} =\frac{y_p-y_l}{p_x^2+p_y^2+1}-\frac{2 p_y \left(-p_xx_l-y_l p_y+p_x x_p+p_y y_p+z_p\right)}{\left(p_x^2+p_y^2+1\right){}^2} = \\
=\frac{y_p-y_l}{p_x^2+p_y^2+1}-\frac{2p_yt_{min}}{p_x^2+p_y^2+1},
\end{split}
\end{equation}

\begin{equation}
\frac{\partial t_{min}}{\partial x_p} = \frac{p_x}{p_x^2+p_y^2+1}
\end{equation}


\begin{equation}
\frac{\partial t_{min}}{\partial y_p} = \frac{p_y}{p_x^2+p_y^2+1}
\end{equation}

\begin{equation}
\frac{\partial t_{min}}{\partial z_p} = \frac{1}{p_x^2+p_y^2+1}
\end{equation}

We can now use product and chain rules with implicit differentiation to obtain the desired residuals from Eq.~\ref{eq:t}
\begin{equation}
\begin{split}
DCA(p_x, p_y, x_L, x_L, x_P, y_P, z_P) = \\ = \sqrt{ (x_L - x_P + p_xt_{min})^2 + (y_L - y_P + p_yt_{min})^2  + (-z_P + t_{min})^2}.
\label{eq:d}
\end{split}
\end{equation}
...
\begin{equation}
\begin{split}
\frac{\partial DCA^2}{\partial x_L}  = 2 DCA \frac{\partial DCA}{\partial x_L} =  2 \left(x_L+t_{\min } p_x-x_P\right),
\end{split}
\end{equation}
Hence, 
\begin{equation}
\begin{split}
\frac{\partial DCA}{\partial x_L} = \frac{\partial DCA^2}{\partial x_L} \times \frac{1}{2DCA} = \frac{ x_L+t_{\min } p_x-x_P }{\sqrt{(x_L - x_P + p_xt_{min})^2 + (y_L - y_P + p_yt_{min})^2  + (-z_P + t_{min})^2 }}
\end{split}
\end{equation}
...
\begin{equation}
\begin{split}
\frac{\partial DCA^2}{\partial p_x}  = 2 DCA \frac{\partial DCA}{\partial x_p} =  2 t_{\min } \left(x_L+t_{\min } p_x-x_P\right).
\end{split}
\end{equation}
...
\begin{equation}
\begin{split}
\frac{\partial DCA}{\partial p_x} = \frac{\partial DCA^2}{\partial p_x} \times \frac{1}{2DCA} = \frac{ t_{\min } \left(x_L+t_{\min } p_x-x_P\right)}{\sqrt{(x_L - x_P + p_xt_{min})^2 + (y_L - y_P + p_yt_{min})^2  + (-z_P + t_{min})^2 }}
\end{split}
\end{equation}
...
\subsection{Improper 3D Misalignment: translations only - implicit derivatives}

\begin{equation}
\begin{split}
\mathrm{Residual} = R = \mathrm{Point \ To \ Line \ DCA(x_P, y_P, z_P, p_x, p_y, x_L, x_L)} -r = \\
=\sqrt{ (x_L - x_P + p_xt_{min})^2 + (y_L - y_P + p_yt_{min})^2  + (-z_P + t_{min})^2} -r.
\end{split}
\end{equation}
with $r$ defined as the DCA of the hit (from measurement, i.e. \say{drift cylinder}). 
...
\begin{equation}
DLC_2 = \frac{ \partial R}{\partial y_L} = 
\end{equation}

\begin{equation}
DLC_3 = \frac{ \partial R}{\partial p_x}  = \frac{\partial DCA}{\partial p_x} = \frac{ t_{\min } \left(x_L+t_{\min } p_x-x_P\right)}{\sqrt{(x_L - x_P + p_xt_{min})^2 + (y_L - y_P + p_yt_{min})^2  + (-z_P + t_{min})^2 }}
\end{equation}

\begin{equation}
DLC_4 = \frac{ \partial R}{\partial p_y} 
\end{equation}


\begin{equation}	
DGL_1 = \frac{\partial R}{\partial x} 
\end{equation}

\begin{equation}	
DGL_2 = \frac{\partial R}{\partial y}
\end{equation}

\begin{equation}	
DGL_3 = \frac{\partial R}{\partial z} 
\end{equation}




\subsection{Improper 3D Misalignment: translations only - explicit derivatives}
...
\begin{equation}
\begin{split}
DLC_1 = \frac{\partial R}{\partial x_L}  = \frac{\partial R}{\partial t_{min}} \times \frac{\partial t_{min}}{\partial x_L} + \frac{\partial R}{\partial x_L} = \\
= \frac{p_x \left(x_L-x_P\right)+y_L p_y+t_{\min } \left(p_x^2+p_y^2+1\right)-p_y y_P-z_P}{\sqrt{\left(x_L+t_{\min } p_x-x_P\right){}^2+\left(y_L+t_{\min } p_y-y_P\right){}^2+\left(t_{\min }-z_P\right){}^2}} \times \\
\times \frac{-p_x}{p_x^2+p_y^2+1} \times \\
\times \frac{x_L+t_{\min } p_x-x_P}{\sqrt{\left(x_L+t_{\min } p_x-x_P\right){}^2+\left(y_L+t_{\min } p_y-y_P\right){}^2+\left(t_{\min }-z_P\right){}^2}} = \\ 
= -\frac{p_x \left(x_L+t_{\min } p_x-x_P\right) \left(p_x\left(x_L-x_P\right)+y_L p_y+t_{\min }\left(p_x^2+p_y^2+1\right)-p_y y_P-z_P\right)}{\left(p_x^2+p_y^2+1\right) \left(^2 \left(x_L+t_{\min } p_x-x_P\right)+^2 \left(y_L+t_{\min } p_y-y_P\right)+^2 \left(t_{\min }-z_P\right)\right)}.
\end{split}
\end{equation}
...
\begin{equation}
DLC_3 = \frac{ \partial R}{\partial p_x}
\end{equation}
...
\subsection{Improper 3D Misalignment: translations only - direct derivatives}

\begin{equation}
\begin{split}
\mathrm{Residual} = R = \mathrm{Point \ To \ Line \ DCA(x_P, y_P, z_P, p_x, p_y, x_L, x_L)} -r = \\
=\sqrt{(x_L - x_P + p_x\frac{p_x x_p + p_yy_p +z_p-p_xx_l-p_yy_l}{p^2_x+p^2_y+1})^2 + }\\ 
\overline{\rule{0pt}{2.5ex}{} + (y_L - y_P + p_y\frac{p_x x_p + p_yy_p +z_p-p_xx_l-p_yy_l}{p^2_x+p^2_y+1})^2  +} \\
\overline{\rule{0pt}{2.5ex}{} + (-z_P + \frac{p_x x_p + p_yy_p +z_p-p_xx_l-p_yy_l}{p^2_x+p^2_y+1})^2} -r.
\end{split}
\end{equation}
with $r$ defined as the DCA of the hit (from measurement, i.e. \say{drift cylinder}). 
...
\begin{equation}
DLC_1 = \frac{\partial R}{\partial x_L}  = \frac{p_x  t_{min} + x_L - x_P}
{\sqrt{\left(p_x  t_{min}+x_L-x_P\right){}^2  + 
\left(p_y  t_{min}+y_L-y_P\right){}^2  +
\left( t_{min}-z_P\right){}^2}} 
\end{equation}

\begin{equation}
DLC_2 = \frac{ \partial R}{\partial y_L} = \frac{p_y  t_{min} + y_L - y_P}
{\sqrt{\left(p_x  t_{min}+x_L-x_P\right){}^2  + 
\left(p_y  t_{min}+y_L-y_P\right){}^2  +
\left( t_{min}-z_P\right){}^2}} 
\end{equation}
...
The DCA between the track and the wire given by
\begin{equation}
DCA = || \textbf{W} - \textbf{T} || = || \begin{pmatrix}x_W - x_T\\y_W - y_T\\z_W - z_T\\\end{pmatrix} || = \sqrt{ (x_W - x_T)^2 + (y_W - y_T)^2 + (z_W - z_T)^2},
\end{equation}
where \textbf{W} and \textbf{T} are the points of closest approach between the wire and the track, respectively. With vector $\bf{WT}$ perpendicular to both the wire and the track. In order to form the required derivatives w.r.t local and global parameters, we need a functional form of DCA expressed in term of the reconstructed track parameters and the position of the straw as well as its gradient? 
\begin{figure}[!ht]
\centering
...
\end{figure}
\begin{equation}
DCA = DCA (p_x, p_y, x_T, y_T, x_W, y_W, z_W).
\end{equation}

There is a clear correlation between the CA on the wire with track parameters, which need to be considered...
Try to write correlation matrix, and residual can be expressed through chain product..
...
Do I need definitions of global and local coordinate frames (see rotations)
Consider DCA sign... from code 

Now we need to minimise two parameters $t_{min}$ and $s_{min}$ for the track and the wire, respectively.

\begin{equation}
t_{min} = \frac{-\textbf{d}\cdot\textbf{u} +(\textbf{d}\cdot\textbf{v})(\textbf{u}\cdot\textbf{v}) }{1 - (\textbf{u}\cdot\textbf{v})^2},
\end{equation}
and 
\begin{equation}
s_{min} = \frac{\textbf{d}\cdot\textbf{u} -(\textbf{d}\cdot\textbf{v})(\textbf{u}\cdot\textbf{v}) }{1 - (\textbf{u}\cdot\textbf{v})^2},
\end{equation}
where $\textbf{d}=W_0 -T_0 =\begin{pmatrix}x_W - x_T\\y_W - x_T\\z_W - z_T\\\end{pmatrix}$, with $W_0$ and $T_0$ being some pints on the wire and the track, respectively.
...
\begin{equation}
\begin{split}
\mathrm{Residual} = R = \mathrm{LineToLineDCA(x, y, z, m_x, m_y, y_0, x_0)} 
\end{split}
\end{equation}
...
\begin{equation}
DLC_1 = \frac{\partial R}{\partial x_0}  
\end{equation}

\begin{equation}
DLC_2 = \frac{ \partial R}{\partial y_0} 
\end{equation}
