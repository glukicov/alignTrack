...
\begin{equation}
\textbf{r}(t) = \begin{pmatrix}x\\y\\z\\\end{pmatrix} = \begin{pmatrix}x_L + m_x  t\\y_L + m_y t\\z_L + m_z t\\\end{pmatrix},
\end{equation}
...
\begin{equation}
\textbf{r}(t) = \begin{pmatrix}x\\y\\z\\\end{pmatrix} = \begin{pmatrix}x_L + \frac{p_x}{p_z}  t\\y_L + \frac{p_y}{p_z}  t\\  t\\\end{pmatrix} = \begin{pmatrix}x_L + p_x^{'} t\\y_L + p_y^{'}  t\\  t\\\end{pmatrix},
\end{equation}
where $(x_L$, $y_L$, $0$) is point where track intersected the Z=0 plane, in the future, we drop the $^{'}$ notation on momentum components.
...
\end{split}
\end{equation}
However, the above forms in Eq.~\ref{eq:DCA2Full} or Eq~\ref{eq:DCA} will produce cumbersome-looking derivatives. Therefore, will will work with Eq.~\ref{eq:DCA2}, and utilising the product and chain rules, for example:
...
Now, let's calculate all derivatives of a form $\frac{\partial DCA^2}{\partial P}$:
\begin{equation}
\begin{split}
...
First, let's consider the sign of DCA. 
