...
class HitEvent(TreeModel):
    ...
    fittedMod1Alignment = DoubleCol()
    trueMod1Alignment = DoubleCol()
    fittedMod2Alignment = DoubleCol()
    trueMod2Alignment = DoubleCol()
...
module_1_alignment = 1.5 # Alignment of first module (in mm)
module_2_alignment = -1.2 # Alignment of second module (in mm)
...
argv = sys.argv[1:]
try:
    opts, args = getopt.getopt(argv, "h:f:t:s:m:q:a1:a2:o:", ["help", "fit_count=", "track_count=", "smearing=", "missing_hits=", "batch_queue=", "module_1_alignment=", "module_2_alignment=", "output_dir="])
except  getopt.GetoptError:
    print "multi_fit.py -f <fit_count> -t <track_count> -s <smearing> -m <missing_hits> -q <batch_queue> -a1 <module_1_alignment> -a2 <module_2_alignment> -o <output_dir>"
    sys.exit(2)
...
for opt, arg in opts:
    if opt in ("-h", "--help"):
        ...
        if (arg in ("short", "medium", "long")):
            ...
            sys.exit()
    elif opt in ("-a1", "--module_1_alignment"):
        module_1_alignment = float(arg)
    elif opt in ("-a2", "--module_2_alignment"):
        module_2_alignment = float(arg)
...
for k in xrange(fit_count):
    ...
    for event_num in xrange(len(events)):
        ...
        for j in xrange(len(true_hit_rads)):
            ...
            t.trueMod1Alignment = module_1_alignment
            t.fittedMod1Alignment = popt[-2]
            t.trueMod2Alignment = module_2_alignment
            t.fittedMod2Alignment = popt[-1]
