...
track_count = 10 # Number of tracks to fit
...
# Contain calculated chi^2 values, and alignment residuals 
alignment_errors = dict()
chi_squared_vals = dict()
...
for k in xrange(fit_count):
    ...
    if (chi_squared < reduced_chi_squared_cut * deg_freedom):
        ...
        if (deg_freedom in alignment_errors):
            chi_squared_vals[deg_freedom].append(chi_squared) 
            alignment_errors[deg_freedom].append(popt[-1] - module_alignment)
        else:
            chi_squared_vals[deg_freedom] = [chi_squared]
...
for deg_freedom in alignment_errors:
    ...
    print "*******************************************"
    print ""
    print "Mean Alignment Error:", np.mean(np.array(alignment_errors[deg_freedom])), "mm"
    print "Standard Deviation:", np.std(np.array(alignment_errors[deg_freedom])), "mm"

    # Plot histogram of residuals
    plt.hist(alignment_errors[deg_freedom], bins=20)
    ...
    number_patch = mpatches.Patch(color='none', label=("Data Count: " + str(len(alignment_errors[deg_freedom]))))
    mean_patch = mpatches.Patch(color='none', label=("$\mu$: " + str(np.mean(np.array(alignment_errors[deg_freedom])))))
    std_patch = mpatches.Patch(color='none', label=("$\sigma$: " + str(np.std(np.array(alignment_errors[deg_freedom])))))


    align_handles = [number_patch, mean_patch, std_patch]
    align_labels = [number_patch.get_label(), mean_patch.get_label(), std_patch.get_label()]
    ...
    chi_bin_conts, chi_bin_edges, chi_patches = plt.hist(chi_squared_vals[deg_freedom], bins=20, label='Calculated Chi-Squared')
    ...
    chi_dist_plot = plt.plot(x_chi_plot, len(chi_squared_vals[deg_freedom]) * chi_bin_width * stats.chi2.pdf(x_chi_plot, deg_freedom), 'r-', label=("Chi-Squared Dist (" + str(deg_freedom) + " dof)"))
    ...
    chi_func_bins_unfiltered = np.array([len(chi_squared_vals[deg_freedom]) * (stats.chi2.cdf(chi_bin_edges[i+1], deg_freedom) - stats.chi2.cdf(chi_bin_edges[i], deg_freedom)) for i in xrange(len(chi_bin_conts))])
    ...
    number_patch = mpatches.Patch(color='none', label=("Data Count: " + str(len(chi_squared_vals[deg_freedom]))))
