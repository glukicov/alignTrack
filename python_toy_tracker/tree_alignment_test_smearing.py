...
argv = sys.argv[1:]
try:
    opts, args = getopt.getopt(argv, "h:i:t:o:m:f:r:c:", ["help", "input_dir=", "track_count=", "output_dir=", "missing_straws=", "fits_attempted=", "hit_resolution=", "chi_sq_cut="])
except getopt.GetoptError:
    print "tree_reader.py -i <input_dir> -t <track_count> -f <fit_count> -c <chi_sq_cut> -o <output_dir> -m <missing_straws> -r <hit_resolution> -c <chi_sq_cut>"
    sys.exit(2)
...
for opt, arg in opts:
    ...
    if opt in ("-h", "--help"):
        print "tree_reader.py -i <input_dir> -t <track_count> -o <output_dir> -r <hit_resolution> -c <chi_sq_cut>"
        sys.exit()
    ...
    elif opt in ("-r", "--hit_resolution"):
        hit_resolution = float(arg)
...
input_file = input_dir + "smear_" + straw_status + "_" + str(track_count) + "_track_" + str(fits_attempted) + "_fit_" + str(hit_resolution) + "_res.root"
...
for i in xrange(fit_count):
    ...
    sigmas = np.array([hit_resolution for n in xrange(fit_cut_tree_entry_count)])
...
plt.title("Plot of Module Misalignment Distances for Fits With All D.o.F., and a Hit Resolution of " + str(hit_resolution) + " mm")
...
# Save plot
plt.savefig(output_dir + straw_status + "_" + str(track_count) + "t_" + str(hit_resolution) + "r_align.pdf")
plt.clf()
...
for dof in chi_sq_vals:
    ...
    plt.ylabel("Frequency")
    plt.title("Plot of $\chi^2$ for Fits With " + str(dof) + " D.o.F., and a Hit Resolution of " + str(hit_resolution) + " Compared to Ideal Distribution")
    
    # Save plot
    plt.savefig(output_dir + straw_status + "_" + str(track_count) + "t_" + str(hit_resolution) + "r_chi" + str(dof) + ".pdf")
    plt.clf()
