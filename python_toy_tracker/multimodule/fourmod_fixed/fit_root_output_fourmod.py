import toytracebackfourmod as ttb
...
argv = sys.argv[1:]
try:
    opts, args = getopt.getopt(argv, "h:f:t:s:m:q:a:o:r:x:", ["help", "fit_count=", "track_count=", "smearing=", "missing_hits=", "batch_queue=", "module_alignment=", "output_dir=", "hit_resolution=", "fitting_algorithm="])
except  getopt.GetoptError:
    print "multi_fit.py -f <fit_count> -t <track_count> -s <smearing> -m <missing_hits> -q <batch_queue> -a <module_alignment> -o <output_dir> -r <hit_resolution> -x <fitting_algorithm>"
    sys.exit(2)

# Set parameters according to arguments
for opt, arg in opts:
    if opt in ("-h", "--help"):
        print "multi_fit.py -f <fit_count> -t <track_count> -s <smearing> -m <missing_hits> -q <batch_queue> -a <module_alignment> -o <output_dir> -r <hit_resolution>"
        sys.exit()
        ...
        ttb.hit_resolution = float(arg)
    elif opt in ("-x", "--fitting_algorithm"):
        fitting_algorithm = arg

print ttb.hit_resolution
...
file_name = directory + smear_string + "_" + straw_string + "_" + str(track_count) + "_track_" + str(fit_count) + "_fit_" + str(ttb.hit_resolution) +"_res.root"
...
for k in xrange(fit_count):
    ...
    popt, pcov = curve_fit(fitting_detector.get_hit_radius, wire_keys, hit_rads, p0=guess, sigma=fit_sigmas, method=fitting_algorithm)
